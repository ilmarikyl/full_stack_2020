...
describe('blog with most likes when', () => {

	test('when list has only one blog', () => {
		expect(listHelper.favouriteBlog(listWithOneBlog)).toEqual(listWithOneBlog[0])
	})

	test('is calculated right when list has several blogs', () => {
	...
	test('when list is empty', () => {
		expect(listHelper.favouriteBlog([])).toBe('Blog list is empty')
	})
})
...
describe('most blogs', () => {
	...
	test('when list is empty', () => {
		expect(listHelper.mostBlogs([])).toBe('Blog list is empty')
	})
})
